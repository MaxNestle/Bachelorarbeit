...
Eine andere Methode, bei der die Integrität jedoch nicht vollständig garantiert, aber simpler umzusetzen ist, ist die Verwendung eines Paritätsbits.\cite{cabuk2004ip}
...
Die letzten beiden Bits dieses Feldes sind ungenutzt und können so zur Datenübertragung verwendet werden. (2 Bits)
...
Das Padding und die ungenutzten Bits des ,,Type of Service'' Feldes können auf 0 gesetzt werden.\\
...
\chapter{Bewertung der Lösungsansätze}
...
\chapter{Konstruktion und Aufbau des Covert Channels}
...
Auf der anderen Seite sind UDP Pakete selten geworden, da HTTP/HTTPS und somit die Webserver auf TCP aufbauen. Auch die Streams von Firmen wie Netflix oder YouTube basieren heute alle auf dem TCP/IP Stack.\\
...
In diesem Kapitel sollen die einzelnen Bausteine, die im vorhergehenden Abschnitt beschriebenen wurden, zu einem kompletten System zusammengefügt werden. Der Aufbau dieses Systems dient später als Struktur bei der Programmierung.\\
Im Allgemeinen können hier zwei Systeme entstehen, entweder ein System mit einem aktiven oder passiven Covert Channel.

\subsection{Anforderungen an den Aufbau}
...
\subsection{Lösungsansatz 1: Aktiver Aufbau}
...
\subsection{Lösungsansatz 2: Passiver Aufbau}

Bei der passiven Alternative (\textit{Abbildung 6.2}) verbindet sich der Empfänger über einen Proxy mit einem beliebigen Webserver, der einen konstanten Datenstrom generiert. Dies könnte zum Beispiel ein Video- oder Audiostream sein.\\
...
\chapter{Umsetzung der aktiven Variante}
\section{Fehlerkorrektur}


\subsection{Anforderung an die Fehlerkorrektur}
...
\subsection{Lösungsansatz 1: Paritätsbits}
...
\subsection{Anfoderung an die Hashfunktion}
Die Hashfunktion soll eine Nachricht auf einen eindeutigen Hashwert abbilden. Er muss nicht zur Authentifizierung verwendet werden. Der Wert sollte so klein wie möglich sein, um die Datenmenge gering zu halten.


\subsection{Lösungsansatz 1: MD5-Hash}
...
\subsection{Lösungsansatz 2: Pearson-Hash}

Der Pearson-Algorithmus verwendet eine zufällig initialisierte statische Mapping-Tabelle, um jedes Byte von jedem Hash-Wert auf einen neuen Hash-Wert abzubilden.\cite{davies2010traffic}. Dafür wird jeweils der alte Hash-Wert mit einem Datenwort XOR verknüpft.
Mit diesem simplen Algorithmus lassen sich Hash-Werte mit der Länge von einem Byte erzeugen. Die Mapping-Taballe muss jedoch auf allen beteiligten Systemen gleich sein.
	...
	Den Algorithmus zur Berechnung des Hashs ist in Listing 7.1 gezeigt.
	...
	\begin{lstlisting}[caption=Pearson Hash]
...
\section{Netzwerkprotokoll}
	...
	\subsection{Lösungsansatz 1: HTTP}
	...
	\subsection{Lösungsansatz 2: SMTP}
	...
	\subsection{Lösungsansatz 3: FTP}
...
\section{Server}
	...
	\subsection{Lösungsansatz 1: Java HttpServer}
	...
	Um den Server nutzen zu können, müssen ihm ein oder mehrere HttpHandler hinzugefügt werden. Diese bearbeiten, die Anfragen auf verschiedene URL Pfaden.\\
	...
	\subsection{Lösungsansatz 2: Node.js und Express}
	...
	Ebenfalls von Express stammt das \textit{http} Objekt, welches den Http Server darstellt. Der Server Port kann wie in Listing 7.3 gewählt werden.\\
	...
	\section{Kommunikation des Covert Channel}
	...
	Diese Methoden, auf die mit http zugegriffen werden, sehen nicht vor, dass ein Server selbständig und ohne danach gefragt zu werden Nachrichten an den Client sendet.
	
	\subsection{Anforderung an die Kommunikation des Covert Channels}
	...
	\subsection{Lösungsansatz 1: Websockets}
	...
	\subsection{Lösungsansatz 2: Socket.IO}
	...
	\subsection{Berwertung der Kommunikation des Covert Channels}
	...
	Socket.IO wird in Form des \textit{io} Objekts in den Code eingebunden. Der Verbindungsaufbau ist, wie in Listing 7.4 gezeigt, aufgebaut. Den Events werden ihre jeweiligen Callback-Funktionen zugeordnet.\\
	...
	\subsection{Anforderung an die Kodierung und Dekodierung}
	...
	\subsection{Lösungsansatz 1: Kodierung mit festen Zeitrastern}
	...
	Für diese Art der Codierung ist es unabdingbar, dass die Uhren von Sender und Empfänger möglichst genau übereinstimmen.\\ Zur Veranschaulichung ist die Kodierung in \textit{Abbildung 7.1} schematisch dargestellt.
	...
	\subsection{Lösungsansatz 2: Kodierung basierend auf den Paketabständen}
	...
	Zu sehen ist diese Art der Kodierung in \textit{Abbildung 7.2}
	...
	\subsection{Implementierung der Kodierung}
	...
	Ist eine komplette Datei übertragen, wird eine vorab definierte Pause von einer Sekunde eingelegt. Danach beginnt die Datenübertragung erneut. In \textit{Listing 7.5} ist der Code gezeigt, der diese Kodierung umsetzt.
	...
	\section{Front-End}
		...
		JQuery ist eine JavaScript Bibliothek, die es wesentlich einfacher macht, das HTML-Dokument und den DOM-Baum zu manipulieren und zu verändern. In diesem Kontext wird sie dazu verwendet, um jeweils die aktuelle Uhrzeit dynamisch auf der Seite anzuzeigen.\\
		Zudem kommt es zum Einsatz, um die Eventhandler für Socket.IO einzubinden.
		
		
		\subsection{Socket.IO}
		Der nötige Code wird über ein Script Tag heruntergeladen und entsprechend hinzugefügt. Wie auch serverseitig, kann hier ein SocketIO Objekt generiert werden.
	...
	\section{Clientseitige Auswertung des Covert Channel}
		...
		\subsection{Lösungsansatz 1: Auswertung im Front-End}
		...
		\subsection{Lösungsansatz 2: Auswertung mit einem externen Programm}
...
\section{Client}
	...
	\subsection{Programmiersprache}
		...
		\subsubsection{Lösungsansatz 1: Java}
			...
			Der Java Code wird von einem Compiler in Bytecode umgewandelt. Dieser kann auf jedem Gerät, das die Java Runtime besitzt, ausgeführt werden. Dadurch muss der Code nicht für jedes Gerät compiliert werden.
			\cite{ullenboom2004java}
			
		\subsubsection{Lösungsansatz 2: Python}
	...
	\subsection{Mitschneiden der Datenenpakete}
		...
		\subsubsection{Lösungsansatz 1: tcpdump}
		...
		\subsubsection{Lösungsansatz 2: tshark}
		...
		\subsubsection{Implementierung des Paket-Sniffer}
		...
		Der Programmcode, der den Paket-Sniffer handhabt, ist in \textit{Listing 7.6} gezeigt.
...
\chapter{Optimales Einstellen und Anwendung des aktiven Covert Channel} 
		...
		\section{Verhältnis nicht interpretierbar/ interpretierbar}
		...
		Die Ergebnisse dieses Tests sind in \textit{Tabelle 8.1} zu sehen. Die Spalten zeigen, jeweils die Länge einer langen Pause in Sekunden. Die Zeilen stehen für den prozentualen Anteil an der Pausenlänge, die das Zeitfenster definiert.\\
 ...
 \end{table}
	...
	Diese Erkenntnisse decken sich auch mit dem auf \textit{Bild 4.2} gezeigten Ergebnissen von \cite{cabuk2004ip}
	...
	\section{Anzahl korrekt übertragener Dateien}
	...
	\section{Zeit bis zum ersten korrekten Eintreffen der Datei}
	...
	In \textit{Tabelle 8.3} sind die Durchschnittswerte gezeigt, die sich aus diesem Test ergeben.\\
	...
	Die Kombination aus einem Zeitfenster von 40\% und einer große Pause von 0,05 Sekunden ist die optimale Lösung für diesen Covert Channel.
		...
		\section{Padding zum Synchronisieren}
...
Um dieses Problem zu umgehen, werden 2 Byte Padding, zum Synchronisieren, an den Anfang der Dateien gestellt. Der Abstand zwischen diesen Paketen ist auf 0.01 Sekunden festgelegt. In \textit{Abbildung 8.3} ist zu sehen, dass mindestens 6 Datenpakete benötigt werden, bis sich der Korrekte Paketabstand einstellt.
...
\chapter{Umsetzung der passiven Variante}
	Die in \textit{Abbildung 6.2} dargestellte passive Lösungsvariante des Covert Channels soll in diesem Kapitel betrachtet werden. Diese Variante hat den Vorteil, dass als Server jeder beliebige Webserver verwendet werden kann. Die Codierung wird von einem Proxy realisiert.\\
	...
	\section{Anforderung an den Proxy}
		...
		\section{Lösungsansatz 1: Veränderung eines Open-Source Proxys}
		...
		\section{Lösungsansatz 2: Proxy selbst programmieren}
	...
	\section{Implementierung}
		\subsection{Sockets}
			...
			In \textit{Listing 7.1} ist der Code zu sehen, der die oben beschriebene Handhabung der Sockets übernimmt. Die Liste aller Sockets wird zur Kontrolle an \textit{select} übergeben.
...
\chapter{Anwendung des passiven Covert Channel}
	...
	\section{Probleme bei realen Webseiten}
			...
			\subsection{Parallele Datenabfrage}	
				Wie vorhergehnd bereits beschrieben, öffnet ein Webserver gleich mehrerer Sockets, um parallel Daten zu versenden. Diese Nachrichtenpakete, der einzelnen Sockets können nicht vom Client auseinander gehalten werden, da dieser nur den Source-Port betrachtet, der jedoch bei allen Paketen gleich.\\
				In Bild 10.1 ist ein Ausschnitt aus den Firefox Development Tool zu sehen. Hier wird die Übertragungsdauer der einzelnen Dateien gezeigt. Dabei fällt auf, dass bei der hier verwendeten Webseite der \textit{PH-Weingarten}, 5 CSS Dateien gleichzeitig übertragen werden.\\
		...
		Jedoch besteht anfangs das, im vorhergehenden Kapitel beschriebene, Problem mit der parallelen Datenübertragung. Das Ergebnis ist in \textit{Bild 10.2} zu sehen. Bei einer optimalen Funktion werden den zeitlichen Differenzen links binären Werte zugeordnet. Dies ist hier in vielen Fällen nicht der Fall, was eine Datenübertragung nicht möglich macht.
...
\subsubsection{Beachtung des Destination Ports beim Client}
