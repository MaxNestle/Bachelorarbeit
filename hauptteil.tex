...
\textbf{Manipuliere den Datenstrom des Trägerkanals so, dass die geheimen Daten kodiert werden.}

Schlüssel zur Datenexfiltration:

\textbf{Lese die Zeitabstände der Datenpakete im Trägerkanal und dekodiere diese.}
...
Die Entscheidung Beschränkt sich nun auf einen UDP oder TCP Stream. UDP hat das Problem das es sich hierbei um eine verbindungsloses Protokoll handelt und man nicht sicher sein kann, dass die Daten in der Richtigen Reihenfolge oder überhaupt ankommen. Durch die Verwendung eines zeitlichen Covert Channel muss zwingend ein System zur Sicherstellung der Integrität implementiert werden. Dies würde auch das Problem mit dem UDP Stream regeln.\\
Bei einem TCP Stream wird ausschließlich TLS verwendet, wobei die Daten verschlüsselt werden. In dieser Arbeit, wo die Sicherheit nur durch die Steganographie garantiert werden soll, wäre so einen Verschlüsselung unnötig und würde auch eher Aufmerksamkeit auf sich lenken.\\
Deshalb wird in diesem Projekt ein UDP Stream als Trägerkanal verwendet.
