...
while True:
	...
	self.send(writable)
		...
		\subsubsection{Covert Channel}
			Bei der Erstellung des Covert Channels müssen die Nachrichten an den Client nun zeitlich Verzögert werden. Dabei besteht allerdings das Problem, dass bei dieser Anwendung ein Thread verwendet wird, der gleichzeitig für das Senden und Empfangen zuständig ist.\\ So kann die Lösung der aktiven Variante, wobei der Thread für die benötigte Zeit in \textit{sleep} gesetzt wird nicht verwendet werden.\\
			Aus diesem Grund wird nach jedem Senden an den Client der Zeitstempel abgespeichert. Bevor ein neues Paket gesendet wird kontrolliert, ob die Differenz der aktuellen Zeit und dem letzten Senden der gewünschten Pause entspricht.
			
	\subsection{Verwendung des Testservers}
	
	
	\subsection{Verwendung einer realer Webseite}
	
			\subsubsection{HTTPS}
	
			\subsubsection{Problem mit meherer Sockets}	
				Wie vorhergehnd bereits beschrieben öffnet einen normale Webserver gleich mehrerer Sockets zum um parallel Daten zu empfangen. Diese Nachrichtenpakete, der einzelnen Sockets können nicht vom Client auseinander gehalten werden, da dieser nur den Source-Port betrachtet. Dieser ist jedoch bei allen gleich.\\
				...
				Jede Datei hat einen eigenen Covert-Channel, da der Client jedoch nicht unter ihnen unterscheiden kann. Kommt kein klares Ergebnis an.
				Der Covert Channel funktioniert also nur, wenn nur eine Datei übertragen wird.
		...
		\subsubsection{Generierung von genügend Nachrichten}
			paketgröße
			buffern
